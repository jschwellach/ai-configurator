...
class LibraryManager(LoggerMixin):
    ...
    def load_catalog(self, force_refresh: bool = False, progress_callback: Optional[Callable[[str], None]] = None) -> Optional[LibraryCatalog]:
        ...
        try:
            ...
            self.logger.info(f"Loaded catalog with {catalog.total_configs} configurations in {load_time:.2f}s")
            
            if progress_callback:
                progress_callback(f"Catalog loaded successfully ({catalog.total_configs} configurations)")
    ...
    def search_configurations(
        ...
        """
        Search configurations based on various criteria with performance optimizations.
        
        Args:
            query: Text search query (searches name, description, tags)
            personas: Filter by target personas
            domains: Filter by domain categories
            tags: Filter by tags
            category: Filter by category (contexts, profiles, hooks, mcp_servers)
        ...
        if progress_callback:
            progress_callback("Loading catalog...", 0, 100)
        ...
        if progress_callback:
            progress_callback("Searching configurations...", 25, 100)
        
        results = []
        total_configs = 0
        processed_configs = 0
        
        # First pass: count total configurations for progress tracking
        for category_name, category_data in catalog.categories.dict().items():
        ...
        # Second pass: process configurations with progress updates
        for category_name, category_data in catalog.categories.dict().items():
    ...
    def get_library_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "total_configs": catalog.total_configs,
            "last_updated": catalog.last_updated,
            "personas_count": len(catalog.personas),
            "categories": {}
        }
